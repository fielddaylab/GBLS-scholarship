...
Reads the coded journal article archive manifest, calculates reproducible
metrics for the ~7,200 reference corpus articles (the grey articles in the
metrics explorer), and writes all artifacts to
2_calculated_metrics/reference_corpus_metrics/.

Also regenerates tools/metrics_explorer/reference_corpus_data.js.
...
METRICS_DIR = PROJECT_ROOT / "2_calculated_metrics" / "reference_corpus_metrics"
REFERENCE_JS = PROJECT_ROOT / "tools" / "metrics_explorer" / "reference_corpus_data.js"
REFERENCE_JS_CODER = PROJECT_ROOT / "tools" / "gbls_lit_coder" / "public" / "metrics_explorer" / "reference_corpus_data.js"

IDENTITY_COLS = ["article_id", "year", "decade", "journal", "title"]

# Manifest key -> schema field name
MANIFEST_FIELD_MAP = {
...
# These are constant across the entire reference corpus
FIXED_FIELDS = {
...
# ── Schema parsing (reused from GBLS script) ────────────────────────────────
...
def load_records(manifest_path: Path, schema_fields: list[str]) -> list[dict]:
    """
    Read every line of _manifest.jsonl and return normalized record dicts.
    Each record has:
        article_id, filename, year, decade, journal, title, doi,
        coding_basis, controlled (dict of field -> str | list[str])
    """
    records = []
    ...
    for line in manifest_path.read_text(encoding="utf-8").splitlines():
        ...
        # article_id: 8-10 char hex key from filename, else stem
        key_m = re.search(r"\(([A-F0-9]{8,12})\)$", stem, re.IGNORECASE)
        article_id = key_m.group(1).upper() if key_m else stem

        # Disambiguate duplicate IDs (rare, but guard against it)
        base_id = article_id
        suffix = 1
        ...
        # Controlled fields from manifest
        controlled: dict[str, str | list[str] | None] = {}

        # Fixed fields (same for every reference article)
...
# ── Field helpers (identical to GBLS script) ───────────────────────────────
...
def build_features_long(
    ...
    return _coerce_year(pd.DataFrame(
        rows,
...
def build_feature_matrix(
    ...
    return _coerce_year(pd.DataFrame(rows, columns=IDENTITY_COLS + feature_cols))
...
def build_journal_counts(records: list[dict]) -> pd.DataFrame:
    total = len(records)
...
def validate(
    ...
    for f in METRICS_DIR.glob("*.csv"):
        try:
            ...
            csv_errors.append(f"{f.name}: {exc}")
    issues.extend(csv_errors)
...
def main() -> None:
    print("=" * 60)
    print("GBLS Reference Corpus Metrics Builder")
    print("=" * 60)
    ...
    if not MANIFEST_PATH.exists():
        ...
        sys.exit(1)
    records = load_records(MANIFEST_PATH, schema_fields)
    ...
    articles_df = build_articles_df(records, schema_fields, multi_label_fields)
    feature_matrix_df = build_feature_matrix(records, schema_fields, feature_cols)
    feature_counts_df = build_feature_counts(records, schema_fields)
    journal_counts_df = build_journal_counts(records)
    ...
    for field in schema_fields:
        ...
        save_csv(build_field_matrix(records, field, field_cols), f"{fkey}_matrix.csv")
...
""",
    ...
    # ── Validation ────────────────────────────────────────────────────────
    print("\n[6/8] Validating...")
    report = validate(MANIFEST_PATH, records, articles_df, features_long, feature_counts_df)
    ...
    xlsx_path = METRICS_DIR / "reference_corpus_overview.xlsx"
    write_excel(
    ...
    # ── JS bundle ─────────────────────────────────────────────────────────
    print("\n[8/8] Refreshing JS reference corpus bundle...")
    write_js_bundle(
        records, schema_fields, multi_label_fields,
        dataset_summary, feature_counts_df, pub_year_df,
        feature_year_df, journal_counts_df, REFERENCE_JS,
    )
    print(f"  Wrote {REFERENCE_JS.name}")
    generated_files.append(REFERENCE_JS)
    if REFERENCE_JS_CODER.exists():
        import shutil as _shutil
        _shutil.copy2(REFERENCE_JS, REFERENCE_JS_CODER)
        print(f"  Copied → {REFERENCE_JS_CODER.relative_to(PROJECT_ROOT)}")
    ...
    print(f"  JS bundle written  : {REFERENCE_JS.exists()}")
