...
Parse every coded article summary in `1_coded_summaries`, calculate reproducible article-level and
aggregate metrics, and write every generated metric artifact to `2_calculated_metrics/gbls_corpus_metrics`.
...
- Input: `<project root>/1_coded_summaries`
- Human-authored sources: `<project root>/0_human_sources`
- Output root: `<project root>/2_calculated_metrics`
- Metrics output: `<project root>/2_calculated_metrics/gbls_corpus_metrics`
- JavaScript explorer folder: `<project root>/tools/metrics_explorer`
- JavaScript explorer files: `<project root>/tools/metrics_explorer/index.html`,
  `<project root>/tools/metrics_explorer/metrics_explorer.css`, and
  `<project root>/tools/metrics_explorer/metrics_explorer.js`
- Generated explorer bundle:
  `<project root>/2_calculated_metrics/metrics_explorer_synthesized_data.js`

If the absolute path is unavailable, locate the project root by finding a directory containing both
`1_coded_summaries` and `2_calculated_metrics`. Create `2_calculated_metrics/gbls_corpus_metrics` if it does not exist. Do not search
outside likely workspace or mounted-drive locations
unless necessary.

## Input Rules

1. Read every `.md` file directly inside `1_coded_summaries`.
...
9. At run time, enumerate the non-hidden documents directly inside
   `0_human_sources` and identify by contents the document that defines the
...
Write all files in this section to `<project root>/2_calculated_metrics/gbls_corpus_metrics`. Write UTF-8 CSV files with
...
Create or replace these metric-specific support files inside `2_calculated_metrics/gbls_corpus_metrics`:
...
Create or replace `2_calculated_metrics/gbls_corpus_metrics/gbls_feature_overview.xlsx` with clean, machine-readable sheets:
...
If `<project root>/tools/metrics_explorer/index.html` and
`<project root>/tools/metrics_explorer/metrics_explorer.js` exist:

1. Regenerate `2_calculated_metrics/metrics_explorer_synthesized_data.js` from
   the newly calculated files in `2_calculated_metrics/gbls_corpus_metrics`.
...
6. Verify that `tools/metrics_explorer/index.html` loads with no console errors and shows the newly
   calculated total article count.

The explorer HTML, CSS, application script, and README are siblings in the project-root
`tools/metrics_explorer` folder. The generated data bundle is directly under
`2_calculated_metrics`. Keep these references in
`tools/metrics_explorer/index.html` valid:

- `metrics_explorer.css`
- `metrics_explorer.js`
- `../../2_calculated_metrics/metrics_explorer_synthesized_data.js`

The standalone explorer's article links must target the relocated coding
application's local development URL, `http://localhost:8787/`. Preserve
`window.GBLS_CODING_TOOL_URL = "http://localhost:8787/";` in this standalone
HTML file. The copy bundled inside `tools/gbls_lit_coder/public` may continue
to use the Worker root because it is served by that application.
...
- Write generated metric artifacts only inside `2_calculated_metrics/gbls_corpus_metrics`.
...
- Do not overwrite unrelated files in `2_calculated_metrics`, including the
  sibling `library_journal_reference_metrics` package.
- Keep the explorer HTML, CSS, application script, and README inside the project-root
  `tools/metrics_explorer` folder. Do not move them into
  `2_calculated_metrics/gbls_corpus_metrics`; the generated
  `metrics_explorer_synthesized_data.js` bundle belongs directly in
...
- Exact metrics output folder (`2_calculated_metrics/gbls_corpus_metrics`)
