...
- Final manuscript drafts: `2-outputs`
- Audits and synthetic reviews: `2-outputs/audits_and_synthetic_reviews`
...
Record each baseline passage in the in-memory working ledger using its source heading
...
Maintain an in-memory working source ledger while drafting. Do not write it as
a standalone file. For every coded summary reviewed, record the summary
filename and classify it as cited,
...
Return the completed section text and working ledger to the master
orchestrator. The orchestrator must retain them in context for later assembly
and audit generation. Do not write a standalone section or ledger file.
