...
REPORT_FILE: ${PROJECT_ROOT}/2-outputs/audits_and_synthetic_reviews/google_scholar_discovery_report.md
```
...
1. Enumerate non-hidden documents in `${PROJECT_ROOT}/0-human-sources`.
...
5. Inspect `${PROJECT_ROOT}/1-coded-summaries` only for duplicate detection
...
5. Also index identifiers and normalized citations from every Markdown file
   in `${PROJECT_ROOT}/1-coded-summaries`.
